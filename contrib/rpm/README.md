...
The RPM spec file provided here is for KenFMcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://kenfmcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[KenFMcoin git repository](https://gitlab.com/kenfmcoin/kenfmcoin) but are not part of
...
Currently only `Source10` is used, for the example `kenfmcoin.conf` file.
...
in the kenfmcoin git repository. At present only an SVG version of the KenFMcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the kenfmcoin
...
needed when 0.12.1 is released, a proper fix is already in the KenFMcoin git
master branch.
...
idea to build KenFMcoin Core against a static version of that library compiled
...
The `kenfmcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `kenfmcoin-server` package. At this point, most RPM based
...
The `kenfmcoin-server` package should have SELinux support. How to properly do
...
KenFMcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/kenfmcoin/). He has been
packaging KenFMcoin for Fedora at least since 2012.
...
4. I renamed the `kenfmcoin` package that contains the Qt GUI to `kenfmcoin-core` as
that appears to be how the general population refers to it, in contrast to
`kenfmcoin-xt` or `kenfmcoin-classic`. I wanted to make sure the general population
