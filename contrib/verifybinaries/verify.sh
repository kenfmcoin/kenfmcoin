...
###   core.kenfmcoin.org and kenfmcoin.org and compares them.
...
WORKINGDIR="/tmp/kenfmcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://core.kenfmcoin.org"
HOST2="https://kenfmcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="kenfmcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'kenfmcoin-',
...
if [ $? -ne 0 ]; then
   echo "kenfmcoin.org failed to provide signature file, but core.kenfmcoin.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "kenfmcoin.org and core.kenfmcoin.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the KenFMcoin Core binary release signing key installed?"
   fi
