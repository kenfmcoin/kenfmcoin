#!/bin/bash
# Copyright (c) 2013 The KenFMcoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete KenFMcoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no KenFMcoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old KenFMcoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected KenFMcoin 0.7 datadir."
    ;;
  3)
    echo "Detected KenFMcoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected KenFMcoin 0.8 datadir."
    ;;
esac
