...
while true; do
	...
	if [ "$NO_SHA1" = "1" ]; then
		export KENFMCOIN_VERIFY_COMMITS_ALLOW_SHA1=0
	else
		export KENFMCOIN_VERIFY_COMMITS_ALLOW_SHA1=1
	fi

	if [ "${REVSIG_ALLOWED#*$CURRENT_COMMIT}" != "$REVSIG_ALLOWED" ]; then
		export KENFMCOIN_VERIFY_COMMITS_ALLOW_REVSIG=1
	else
		export KENFMCOIN_VERIFY_COMMITS_ALLOW_REVSIG=0
	fi
