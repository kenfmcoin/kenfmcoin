...
*Setup instructions for a Gitian build of KenFMcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the KenFMcoin
...
- [Building KenFMcoin Core](#building-kenfmcoin-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-kenfmcoin-host/README.md).
...
Clone the git repositories for kenfmcoin and Gitian.
...
in the kenfmcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building KenFMcoin Core
----------------

To build KenFMcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the kenfmcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/kenfmcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://gitlab.com/kenfmcoin/kenfmcoin
...
For example:
```bash
URL=https://github.com/laanwj/kenfmcoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit kenfmcoin=${COMMIT} --url kenfmcoin=${URL} ../kenfmcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit kenfmcoin=${COMMIT} --url kenfmcoin=${URL} ../kenfmcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit kenfmcoin=${COMMIT} --url kenfmcoin=${URL} ../kenfmcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the kenfmcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../kenfmcoin/contrib/gitian-descriptors/*|sort|uniq )
...
KENPATH=/some/root/path/kenfmcoin
SIGPATH=/some/root/path/kenfmcoin-detached-sigs

./bin/gbuild --url kenfmcoin=${KENPATH},signature=${SIGPATH} ../kenfmcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/kenfmcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/kenfmcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/kenfmcoin-osx-build.assert
```
...
[kenfmcoin/gitian.sigs](https://gitlab.com/kenfmcoin/gitian.sigs/) repository, or if that's not possible create a pull
